...
func Routers() {
	...
	fmt.Println("New DELETE")

	http.ListenAndServe("localhost:9080",
		&CORSRouterDecorator{router})

	fmt.Println("Routers Done")
}
...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := db.Prepare("INSERT INTO users(firstName," +
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	params := mux.Vars(r)
	result, err := db.Query("SELECT id, firstName,"+
		"lastName,email from users WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func InitDB() {
	db, err = sql.Open("mysql",
		"appuser:Secure@123@tcp(localhost:3306)/testdb")

	if err != nil {
		panic(err.Error())
	}
	fmt.Printf("Init DB Done")
}
